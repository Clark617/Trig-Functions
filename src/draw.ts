type TrigFunc = {
    ...
    freq: number; // cos k*x => k
    function: (x: number) => number;
    color: string;
    name: string;
}

export const trigFuncs: TrigFunc[] = [
    {
        amp: 3,
        freq: 1,
        function: Math.sin,
...
export function drawAllWaves(canvas: HTMLCanvasElement, includeWaves: number[]) {
    ...
    for (let i = 0; i < trigFuncs.length; i++) {
        if (!includeWaves.includes(i)) continue
...
function drawWaveAxes(canvas: HTMLCanvasElement) {
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
...
function drawWave(canvas: HTMLCanvasElement, amplitude: number, freq: number, func: (x: number) => number, color: string = "cyan") {
